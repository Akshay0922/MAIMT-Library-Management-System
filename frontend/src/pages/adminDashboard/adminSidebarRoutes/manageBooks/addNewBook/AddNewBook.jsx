          ...
          >
            {({ handleSubmit, handleChange, handleBlur, values, touched, errors, setFieldValue }) => (
              <Form noValidate onSubmit={handleSubmit} className="form-data">
                ...
                <Row className="mb-3">
                  <Col md={4}><Form.Group><Form.Label>Author</Form.Label>
                    <div className="input-suggestion-wrapper">
                      <Form.Control type="text" name="author" value={values.author} onChange={(e) => handleSuggestion("author", e.target.value, setFieldValue)} onBlur={handleBlur} isInvalid={touched.author && !!errors.author} autoComplete="off" />
                ...
                {/* ROW 6 */}
                <Row className="mb-3">
                  <Col md={4}><Form.Group><Form.Label>Vendor</Form.Label>
                    <div className="input-suggestion-wrapper">
                      <Form.Control type="text" name="vendor" value={values.vendor} onChange={(e) => handleSuggestion("vendor", e.target.value, setFieldValue)} onBlur={handleBlur} isInvalid={touched.vendor && !!errors.vendor} autoComplete="off" />
                      {suggestions.vendor.length > 0 && (
                        <ul className="suggestion-box">
                          {suggestions.vendor.map((v) => (
                            <li key={v._id} onClick={() => handleSelect("vendor", v.name, setFieldValue)}>{v.name}</li>
                          ))}
