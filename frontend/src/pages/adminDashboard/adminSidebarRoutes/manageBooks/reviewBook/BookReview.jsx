...
export const BookReviewPage = () => {
    ...
    const fetchAccessions = async () => {
      ...
      for (let course of courseSet) {
        try {
          ...
          console.error(` Error fetching accession for ${course}:`, err.message);
          tempMap[course] = 0;
        }
      }
      ...
      // Expand books with accession numbers
      const expanded = local.flatMap((book) => {
        const count = Number(book.noOfBooks) || 1;
        const startAcc = tempMap[book.course] || 0;
        return Array.from({ length: count }, (_, i) => ({
          ...book,
          copyIndex: i + 1,
          accessionNumber: startAcc + i + 1,
          costPerBook: book.cost || 0
        }));
      });
  ...
  const handleFinalSubmit = async () => {
    ...
    for (let book of books) {
      if (!book.title?.trim() || !book.isbn?.trim() || !book.author?.trim() || !book.publisher?.trim()) {
        toast.warn(`Skipping book "${book.title || "Untitled"}": missing required fields.`);
        continue;
      }
      ...
      const formatted = {
        ...
        department: { name: book.department.trim() },
        course: { name: book.course.trim() },
        noOfBooks: 1, // Each row = one book
        accessionNumber: Number(book.accessionNumber),
        rackNo: book.rackNumber,
        shelfNo: book.shelfNo,
        year: Number(book.year),
        cost: Number(book.cost),
        vendorBillId: book.vendorBillId || null
      };
    ...
    if (failedBooks.length === 0) {
      ...
      toast.error(`${failedBooks.length} book(s) failed.`);
    }
  };
            ...
            <Table striped bordered hover responsive style={{ minWidth: "1800px" }}>
              ...
              <tbody>
                {books.map((book, i) => (
                  <tr key={i}>
                    <td>{i + 1}</td>
                    <td><Form.Control value={book.isbn} onChange={(e) => handleInputChange(i, "isbn", e.target.value)} /></td>
                    <td><Form.Control value={book.title} onChange={(e) => handleInputChange(i, "title", e.target.value)} /></td>
                    <td><Form.Control value={book.author} onChange={(e) => handleInputChange(i, "author", e.target.value)} /></td>
                    <td><Form.Control value={book.edition} onChange={(e) => handleInputChange(i, "edition", e.target.value)} /></td>
                    <td><Form.Control value={book.department} onChange={(e) => handleInputChange(i, "department", e.target.value)} /></td>
                    <td><Form.Control value={book.accessionNumber} onChange={(e) => handleInputChange(i, "accessionNumber", e.target.value)} /></td>
                    <td><Form.Control value={book.course} onChange={(e) => handleInputChange(i, "course", e.target.value)} /></td>
                    <td><Form.Control value={book.pages} onChange={(e) => handleInputChange(i, "pages", e.target.value)} /></td>
                    <td><Form.Control value={book.rackNumber} onChange={(e) => handleInputChange(i, "rackNumber", e.target.value)} /></td>
                    <td><Form.Control value={book.shelfNo} onChange={(e) => handleInputChange(i, "shelfNo", e.target.value)} /></td>
                    <td><Form.Control value={book.publisher} onChange={(e) => handleInputChange(i, "publisher", e.target.value)} /></td>
                    <td><Form.Control value={book.place} onChange={(e) => handleInputChange(i, "place", e.target.value)} /></td>
                    <td><Form.Control value={book.year} onChange={(e) => handleInputChange(i, "year", e.target.value)} /></td>
                    <td><Form.Control value={book.cost} onChange={(e) => handleInputChange(i, "cost", e.target.value)} /></td>
                    <td><Button variant="danger" size="sm" onClick={() => handleDelete(i)}>Delete</Button></td>
                  </tr>
                ))}
              </tbody>
            </Table>
